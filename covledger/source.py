...
def classify_source(path: Path, *, root: Path | None = None, excludes: tuple[str, ...] = ()) -> SourceClassification:
    ...
    for pattern in (*DEFAULT_GENERATED_PATTERNS, *excludes):
...
def discover_python_files(
    target: Path, *, include_generated: bool = False, excludes: tuple[str, ...] = ()
 ) -> list[Path]:
    ...
    if target.is_file():
        ...
        classification = classify_source(target, root=target.parent, excludes=excludes)
        return [target] if include_generated or classification.included_in_priority else []
    files: list[Path] = []
    for path in target.rglob("*.py"):
        ...
        if relative.parts and (relative.parts[0] == "tests" or path.name.startswith("test_")):
            continue
        classification = classify_source(path, root=target, excludes=excludes)
        if not include_generated and not classification.included_in_priority:
            continue
        files.append(path)
